	...
	public static void main(String[] args)
	{
		...
		while (scanner.hasNextLine())
		{
			...
			else if (action.equalsIgnoreCase("NEWCUST")) // Create a new registered customer
			{
				...
				// Reads in name from user input
				System.out.print("Name: ");
				if (scanner.hasNextLine())
					name = scanner.nextLine();
				
				// Reads in address from user input
				System.out.print("\nAddress: ");
				if (scanner.hasNextLine())
			...
			else if (action.equalsIgnoreCase("ORDER")) // order a product for a certain customer
			{
				...
				String order1 = amazon.orderProduct(productId, customerId, "");
				// Print Order Number string returned from method in ECommerceSystem
				if (order1 != null) {
					System.out.println("Order # " + order1);
				}
				else {
					System.out.println(amazon.getErrorMessage()); // prints error message if product or customer do not exist
				}
			}
			else if (action.equalsIgnoreCase("ORDERBOOK")) // order a book for a customer, provide a format (Paperback, Hardcover or EBook)
			{
				...
				String order2 = amazon.orderProduct(productId, customerId, options);
			...
			else if (action.equalsIgnoreCase("ORDERSHOES")) // order shoes for a customer, provide size and color 
			{
				...
				options +=  " " + scanner.nextLine();
				//order shoes
				String order_shoe = amazon.orderProduct(productId, customerId, options);
				if (order_shoe != null) {
					System.out.println("Order # " + order_shoe);
				}
				else {
					System.out.println(amazon.getErrorMessage());
				}
			...
			else if (action.equalsIgnoreCase("CANCEL")) // Cancel an existing order
			{
				...
				boolean ord = amazon.cancelOrder(orderNumber);
				if (ord == false) {
					System.out.println(amazon.getErrorMessage());
				}
